...
def rawDataInsert(rawData):
	...
	insert_flow_data = ("Insert into water_Meter_Raw (recordDate, volume) Values (%(date)s, %(volume)s);")
...
while True:
    try:
        ...
        time.sleep(0.5)
        start_counter = 0
        flow = (count / 7.5) # Pulse frequency (Hz) = 7.5Q, Q is flow rate in L/min. flow = (count / 7.5)
        if flow > 0:
            ...
            flow_data = { 'date': currentDate, 'volume': flowm3}
